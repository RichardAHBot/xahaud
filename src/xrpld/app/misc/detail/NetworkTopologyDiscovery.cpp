...
#include <xrpl/crypto/cryptoprov.h>
...
{
    auto& nodeIdentity = app_.nodeIdentity();
    auto const publicKey = nodeIdentity.publicKey;

    // Convert Ed25519 public key to bytes
    std::vector<uint8_t> nodeId(32);
    publicKey.copyBytes(nodeId.data(), nodeId.size());
    ...
    app_.peerfinder().machine_gun_highway_peers(slice, uint256{});
}
